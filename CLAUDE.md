...
- **Deploying on the Pi runs from a venv, not system Python**: `gh.service`'s `ExecStart` uses `/home/pi/kivy/.venv/bin/python3`. Modern Raspberry Pi OS (Bookworm+) blocks `pip3 install <pkg>` system-wide as an "externally-managed-environment" (PEP 668) — install/update dependencies into that venv instead: `/home/pi/kivy/.venv/bin/pip3 install <pkg>`. After changing dependencies or code, `sudo systemctl restart gh.service`; tail logs with `journalctl -u gh.service -f`.
...
- Config lives in `mqtt_config.json` (repo root, gitignored since it holds a plaintext credential — written with `enabled: false` defaults on first run if missing, or copy `mqtt_config.example.json` to `mqtt_config.json` and fill it in). Set `enabled: true` and fill in `host`/`port`/`username`/`password` to turn it on; on Home Assistant OS/Supervised, `host` is the HA host running the Mosquitto broker add-on, and `username`/`password` are a dedicated MQTT login added under the Mosquitto add-on's `logins` config (not the HA user login).
