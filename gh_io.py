...
def gh_io_test():
    ...
    io_manager=IO_Thread_Manager(sim_hw=False)  #sim_hw for the Manager determinest whether to start pigpio
    
    '''
    #Create one thread from the base class
    io_thread1=IO_Thread(threadname="Thread 1", \
                         out_q=local_io_q, \
                         sim_hw=True, \
                         period=1)
    ...
    io_thread2=IO_Thread_ExampleIO(threadname="Thread 2", \
                         out_q=local_io_q, \
                         sim_hw=True, \
                         period=1.5)
    ...
    while((datetime.now()-time1).total_seconds()<10): #run for 10 sec
